...
| **FR-B2** | P0 | Header shows a count — `Hosted zones (12)` — and hosts `Create hosted zone`, `View details`, `Delete`. `[VERIFIED]` against direct capture (`02-zones-list.png`) — the selection-dependent action reads `View details` (routing to the zone detail page), not `Edit`; editing lives on the detail page itself (FR-B21). Selection-dependent actions disabled until a row is selected. Because the total is known only from the response, the counter uses Cloudscape's **server-side** form and is **hidden while loading** rather than flashing a stale number. **Corrected 2026-07-27**: an earlier draft listed the selection-dependent button as `Edit`; the capture showed it reads `View details`. |
...
| ID | Priority | Requirement |
|---|---|---|
| **FR-B21** | P0 | `[VERIFIED]` against direct capture (`04-records-table.png`) — [DD-21](./02-design-decisions.md#dd-21--direct-console-capture-completed-ui-revamped-surface-by-surface-against-it). Cloudscape `Tabs`: **Records** (default), **Accelerated recovery** `[DERIVED — A2]` (shows the demo-limitation toast on selection rather than a working tab — [DD-22](./02-design-decisions.md#dd-22--mocked-actions-get-a-shared-demo-limitation-toast-rather-than-more-coming-soon-routes)), **DNSSEC signing** `[DERIVED — A2]` (placeholder), **Tags** (real — the zone's actual tags, read-only). **"Hosted zone details" is not a tab** — it is an `ExpandableSection` (`variant="container"`, default-expanded) sitting *above* the tab strip, holding the zone's key-value summary (name, record count, created, description, created by, zone ID, nameservers) with an inline **`Edit`** link (routes to FR-B15's edit page). Header actions are four flat buttons — **`Export zone file`**, **`Delete zone`**, **`Test record`** (toast), **`Configure query logging`** (toast) — no `Actions` dropdown. **Corrected 2026-07-27**: an earlier draft (written before direct capture existed) specified "Hosted zone details"/"Query logging" as tabs, an `Actions` dropdown, and standalone `Edit`/`Delete` buttons — the capture showed all three wrong: query logging is a header button not a tab, the details block is an expandable panel not a tab, editing is an inline link not a standalone button, and there is no dropdown. |
...
| ID | Priority | Requirement |
|---|---|---|
| **FR-C3** | P0 | Columns `Record name`, `Type`, `Routing policy` `[DERIVED — A2]`, `Alias` `[DERIVED — A2]`, `Value/Route traffic to`, `TTL (seconds)`. `[VERIFIED]` against direct capture (`04-records-table.png`) — `[UNVERIFIED]` previously. `Differentiator`, `Health check ID`, `Record ID` available but hidden by default, still `[UNVERIFIED]` (not visible in the capture). |
| **FR-C3a** | P1 | `[DERIVED]` **Selecting a record opens an `AppLayout` split panel** — matching `04-records-table.png` — showing "Select a record to see its details" when nothing is selected, "N records selected" plus a prompt when more than one is, and the single selected record's full field set (name, type, routing policy, alias, TTL, values) when exactly one is. Threaded from the Records tab page up to the console shell's `AppLayout` via a dedicated context, since `splitPanel` is a shell-level prop the page itself cannot render directly ([DD-24](./02-design-decisions.md#dd-24--record-detail-split-panel-threaded-via-react-context)). |
...
| **FR-C5** | P0 | **Filter (AS-E8)**: `Record type` multi-select across the nine types, plus `Routing policy` and `Alias` single-selects (matching `04-records-table.png`) — all combining with search and each other as AND, all **server-side** (backend `routing_policy`/`alias` query params, extending §11.1). **Added 2026-07-27**: the Routing policy and Alias filters were added alongside the direct-capture UI revamp — the columns were already real (FR-C3), only the filter controls were missing. |
...
| ID | Priority | Requirement |
|---|---|---|
| **FR-C9** | P0 | `Create record` opens **`Quick create record`** `[VERIFIED]` with the console's exact field labels: `Record name` (editable prefix, with the zone name as **plain text** beside it, not a second disabled input `[VERIFIED]`), `Record type`, `Alias`, **`Value`** (textarea — *"Enter multiple values on separate lines."* `[VERIFIED]`; labelled **`Route traffic to`** instead when Alias is on), `TTL (seconds)` with quick-set **1m / 1h / 1d** `[VERIFIED]` (no `5m`), and `Routing policy`. Type options carry their console descriptions — only **A**'s wording is `[VERIFIED]` against direct capture (`A – Routes traffic to an IPv4 address and some AWS resources`); the remaining eight follow the same phrasing pattern but are best-effort, not directly captured. The submit button reads **`Create records`**, plural `[VERIFIED]`. Multiple values are permitted for **every type except CNAME** `[VERIFIED]`, which collapses to a single input. `[VERIFIED]` against direct capture (`05-create-record-form.png`) — [DD-21](./02-design-decisions.md#dd-21--direct-console-capture-completed-ui-revamped-surface-by-surface-against-it). Full copy deck in [§7 of the UI spec](./07-ui-spec.md). **Corrected 2026-07-27**: an earlier draft had the field labelled `Value/Route traffic to` always, a bordered/disabled second input for the zone suffix, the type descriptions in their short developer-guide form, and a `5m` TTL preset — the capture showed all four wrong. |
| **FR-C9a** | P1 | `[DERIVED]` **Multi-record quick create, built for real** — matching `05-create-record-form.png`'s numbered `Record 1` section and `Add another record` button. The page holds an array of record drafts, each an independently editable, independently deletable `ExpandableSection`; `Create records` submits every draft sequentially against the existing single-record create endpoint (no bulk-create endpoint), stopping and reporting which draft failed if one does. [DD-23](./02-design-decisions.md#dd-23--multi-record-quick-create-built-for-real) |
| **FR-C10** | P2 | A `Switch to wizard` link `[VERIFIED]` toggles the multi-step flow (`Choose routing policy` → `Configure records` → `Review`). **Deferred** — Quick create covers every requirement and is the mode most users work in; the wizard flow itself is not built. **Updated 2026-07-27**: the link is present, matching direct capture (`05-create-record-form.png`), and clicking it shows the shared demo-limitation toast rather than nothing ([DD-22](./02-design-decisions.md#dd-22--mocked-actions-get-a-shared-demo-limitation-toast-rather-than-more-coming-soon-routes)) — the deferral is stated the moment it's hit rather than the link silently doing nothing. |
...
| **FR-E2** | P0 | **Navigation structure (AS-E2)** reproduces Route53's **full** console tree with its section dividers. `[VERIFIED]` against direct capture (`01-nav.png`) — [DD-21](./02-design-decisions.md#dd-21--direct-console-capture-completed-ui-revamped-surface-by-surface-against-it): flat top-level **Dashboard, Hosted zones, Health checks, Profiles**; then sectioned **Global Resolver** (Global resolvers); **VPC Resolver** (VPCs, Inbound endpoints, Outbound endpoints, Rules, Query logging, Outposts); **Domains** (Registered domains, Requests); **IP-based routing** (CIDR collections); **Traffic flow** (Traffic policies, Policy records); then flat **Applications, Test record**. Sections beyond the brief's five are `[DERIVED — A2]`: marginal cost is one component and N routes, and a short nav is among the most obvious tells that a console clone is not the real thing. All leaves except Hosted zones either route to Coming Soon (FR-F1) or, for two leaves added purely for tree completeness (`Global resolvers`, `Outposts`), show the shared demo-limitation toast instead of navigating ([DD-22](./02-design-decisions.md#dd-22--mocked-actions-get-a-shared-demo-limitation-toast-rather-than-more-coming-soon-routes)). Active item highlighted. Full tree in [§3 of the UI spec](./07-ui-spec.md). **Corrected 2026-07-27**: an earlier draft had Profiles nested under *Hosted zones* and Resolver as one undivided section — `01-nav.png` showed both wrong; this row previously carried `[UNVERIFIED ordering]`, now resolved. |
...
| **FR-E12** | P1 | **Unsaved-changes guard.** Navigating away from an edit form with unsaved changes opens a confirmation `Modal` — `Cancel` (`variant="link"`) to stay, primary action to leave. Applies to FR-B15 (edit zone) and FR-C14 (edit record). AWS ships `unsaved-changes-modal` inside its *standard form components*, so this is normal console furniture rather than an extra. Copy is `[UNVERIFIED]` pending the `form-unsaved-changes` demo (UI spec §10). **Status note added 2026-07-27**: not yet implemented — the only P1/P0 requirement in this document not confirmed present in the codebase as of the direct-capture UI revamp ([DD-21](./02-design-decisions.md#dd-21--direct-console-capture-completed-ui-revamped-surface-by-surface-against-it)). Genuinely additive (Stage 2, §11 of the UI spec) rather than structural, so it doesn't block anything else — but it is the one open item worth calling out rather than leaving implicitly covered. |
...
| **FR-F2** | **P1** | **The Dashboard is the post-sign-in landing page**, as it is in the real console — so it is built for real rather than left as a placeholder. `[VERIFIED]` against direct capture (`08-dashboard.png`) — [DD-21](./02-design-decisions.md#dd-21--direct-console-capture-completed-ui-revamped-surface-by-surface-against-it): a four-card feature grid (DNS management, Availability monitoring, Traffic management, Domain registration), a Register-domain search box, and a Notifications table. Only **DNS management → `Create hosted zone`** is a real action; the other three cards, the domain search/transfer link, and the Notifications refresh button show the shared demo-limitation toast ([DD-22](./02-design-decisions.md#dd-22--mocked-actions-get-a-shared-demo-limitation-toast-rather-than-more-coming-soon-routes)). Genuine zone/record counts are shown as secondary text on the DNS management card. Raised from P2 because it is the first screen a reviewer sees, and landing on an empty "Coming Soon" would be a weak opening. Layout in [§5.1 of the UI spec](./07-ui-spec.md). **Corrected 2026-07-27**: an earlier draft (written before direct capture existed) specified `ColumnLayout` stat containers and a recently-created-zones table — direct capture showed the real Dashboard has neither; this row is rewritten to match. |
...
| `GET` | `/hosted-zones/{id}/records` | List — `?search=&type=&routing_policy=&alias=&sort=&order=&page=&page_size=`. **Added 2026-07-27**: `routing_policy` and `alias` extend FR-C5's filters, matching real console columns that already existed |
...
| **NFR-11** | P0 | `JWT_SECRET`, `DATABASE_URL`, `CORS_ORIGINS`, `BACKEND_ORIGIN`, `COOKIE_SECURE` come from the environment, with a committed `.env.example`. No secret committed. **Corrected 2026-07-27**: an earlier draft listed `NEXT_PUBLIC_API_URL`, which implied the frontend calls the backend cross-origin; DD-16's rewrite proxy means the frontend needs `BACKEND_ORIGIN` server-side instead — this now matches architecture §6. **Added 2026-07-27 (Slice 2)**: `COOKIE_SECURE` (default `true`) — an automated auth test over plain HTTP demonstrated that a `Secure` session cookie (`NFR-2`) is silently never returned by the client when the connection isn't HTTPS, so local `http://localhost` development and the test suite need it set `false`; every deployed environment (real HTTPS) leaves it at the default. **Removed 2026-07-28**: `DEMO_MODE` previously gated whether the login page showed the seeded demo credentials. Real testers of the deployed demo couldn't find login credentials — the var was never set in Vercel's project settings, so the build silently omitted them. Since this app mocks auth entirely (`AS-A1`) with no signup path, there is no confidentiality reason to hide them; the credentials Alert is now unconditional and the env var is gone rather than left as a footgun. |
| **NFR-12** | P0 | **AS-D8 + AS-T3 together**: SQLite lives on persistent storage, not an ephemeral container filesystem, so the hosted demo survives redeploys. Frontend on Vercel. **Corrected 2026-07-27**: originally specified as a mounted Fly.io volume ([DD-2](./02-design-decisions.md#dd-2--vercel-for-the-frontend-flyio-with-a-volume-for-the-backend)); the deployed backend runs on PythonAnywhere instead (Fly.io requires a card even for its free allowance), whose per-account filesystem is persistent by default. The Next.js rewrite proxy (DD-16) keeps the session cookie same-origin regardless of which host serves the API, so the cross-origin `SameSite=None`/credentialed-CORS requirement this row originally specified never actually applies in practice. |
...
| **FR-C9a** | Multi-record quick create (real, not cosmetic) | **AS-V1** — matches the real console's own create-record capture | [DD-23](./02-design-decisions.md#dd-23--multi-record-quick-create-built-for-real) |
| **FR-C3a** | Record-detail split panel | **AS-V1** — matches the real console's Records tab capture | [DD-24](./02-design-decisions.md#dd-24--record-detail-split-panel-threaded-via-react-context) |
| **FR-C5** (Routing policy/Alias filters) | Two extra record filters | **AS-E8** — the columns (FR-C3) already existed; the filters complete them | — |
...
- **Direct console capture** (`docs/reference/*.png`) — five screenshots of the live console: nav
  (`01-nav.png`), zones list (`02-zones-list.png`), records table (`04-records-table.png`), create
  record form (`05-create-record-form.png`), Dashboard (`08-dashboard.png`). Source of the structural
  corrections in [DD-21](./02-design-decisions.md#dd-21--direct-console-capture-completed-ui-revamped-surface-by-surface-against-it):
  nav grouping, table column sets, the Dashboard's real layout, the zone-detail tab set, and the
  record-type dropdown's actual option text (which differs from the developer guide's prose). Added
  2026-07-27, completing the plan [DD-19](./02-design-decisions.md#dd-19--how-ui-fidelity-is-sourced) committed to.

**Still `[UNVERIFIED]`**, carried as best-effort per A4: NS record default TTL (`172800`), hosted zone
ID character format, console date format, zone description length cap, and eight of the nine record
type option strings (only **A**'s wording is directly captured). Table column sets and their default
visibility, left-nav tree ordering, and TTL quick-set presets were previously on this list and are
now resolved by direct capture.
