...
| **NFR-11** | P0 | `JWT_SECRET`, `DATABASE_URL`, `CORS_ORIGINS`, `BACKEND_ORIGIN`, `DEMO_MODE` come from the environment, with a committed `.env.example`. No secret committed. **Corrected 2026-07-27**: an earlier draft listed `NEXT_PUBLIC_API_URL`, which implied the frontend calls the backend cross-origin; DD-16's rewrite proxy means the frontend needs `BACKEND_ORIGIN` server-side instead — this now matches architecture §6. |
...
| R2 | **Cloudscape + Next.js App Router** — Cloudscape is client-only React; naive use breaks SSR. Threatens **AS-T1** and **AS-E1**. | Wrap in a `"use client"` shell, import global CSS once at the root layout. Prototype in the first slice, before screens depend on it. |
